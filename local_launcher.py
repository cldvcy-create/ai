# ============================
# Ouroboros — Local launcher (Linux, no Google Colab / Drive)
# ============================
# Drop-in replacement for colab_launcher.py for local Linux testing.
# Storage: ~/.ouroboros/ instead of Google Drive.
# Secrets:  .env file in the same directory as this script.
#
# .env file format (one KEY=VALUE per line):
# ------------------------------------------
...
# OUROBOROS_MODEL_LIGHT=openai/gpt-4o-mini
...
def load_dotenv_file(path: pathlib.Path) -> None:
    """Read a .env file and inject keys into os.environ.

    Rules:
    - Skip blank lines and lines starting with '#'.
    - Strip inline comments (everything after ' #' with a space before '#').
    - Already-set env vars are NOT overwritten (os.environ takes priority).
    """
    if not path.exists():
        print(f"[Ouroboros] WARNING: .env file not found at {path}")
        return
...
load_dotenv_file(_SCRIPT_DIR / ".env")
...
def ensure_claude_code_cli() -> bool:
    """On local Linux, Claude CLI is installed manually. This is a no-op."""
    # To install manually: npm install -g @anthropic-ai/claude-code
...
# ----------------------------
# 1) Secrets + runtime config (local, from .env / os.environ)
# ----------------------------
...
print(f"[Ouroboros] Local launcher — storage: {DRIVE_ROOT}")

for sub in ["state", "logs", "memory", "index", "locks", "archive", "memory/knowledge"]:
